...
class StreamConvertersTest {
  ...
  def streamAccumulate() {
    for (n <- ns) {
      ...
      assert(accO.isInstanceOf[scala.collection.mutable.java8.Accumulator[_]], s"stream $n to generic accumulator")
      ...
      for (boxless <- Seq(false, true)) {
        ...
        assert(accD.isInstanceOf[scala.collection.mutable.java8.DoubleAccumulator], s"double stream $n to generic accumulator $sbox")
        ...
        assert(accI.isInstanceOf[scala.collection.mutable.java8.IntAccumulator], s"int stream $n to generic accumulator $sbox")
        ...
        assert(accL.isInstanceOf[scala.collection.mutable.java8.LongAccumulator], s"long stream $n to generic accumulator $sbox")
      }
    }
  }
