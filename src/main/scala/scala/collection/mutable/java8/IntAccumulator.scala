...
extends AccumulatorLike[Int, IntAccumulator]
{ self =>
  ...
  final def iterator = new Iterator[Int] {
    private var consumed = 0L
    private var hix = 0
    private var ix = 0
    private var i = 0
    private var a: Array[Int] = IntAccumulator.emptyIntArray
  ...
  final def spliterator: java.util.Spliterator.OfInt = new IntAccumulatorSpliterator(this)
...
private[java8] class IntAccumulatorSpliterator(private val acc: IntAccumulator) extends java.util.Spliterator.OfInt {
  ...
  private def duplicateSelf(limit: Long = N): IntAccumulatorSpliterator = {
    val ans = new IntAccumulatorSpliterator(acc)
    ans.h = h
    ans.i = i
    ans.a = a
    ans.n = n
  ...
  def tryAdvance(f: java.util.function.IntConsumer): Boolean = 
    if (N <= 0) false
    else {
      if (i >= n) loadMore()
      f.accept(a(i))
  ...
  def trySplit: java.util.Spliterator.OfInt =
    if (N <= 1) null
    else {
      val half = (N >> 1)
