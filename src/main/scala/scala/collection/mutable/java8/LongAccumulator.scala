...
extends AccumulatorLike[Long, LongAccumulator]
{ self =>
  ...
  final def iterator = new Iterator[Long] {
    private var consumed = 0L
    private var hix = 0
    private var ix = 0
    private var i = 0
    private var a: Array[Long] = LongAccumulator.emptyLongArray
  ...
  final def spliterator: java.util.Spliterator.OfLong = new LongAccumulatorSpliterator(this)
...
private[java8] class LongAccumulatorSpliterator(private val acc: LongAccumulator) extends java.util.Spliterator.OfLong {
  ...
  private def duplicateSelf(limit: Long = N): LongAccumulatorSpliterator = {
    val ans = new LongAccumulatorSpliterator(acc)
    ans.h = h
    ans.i = i
    ans.a = a
    ans.n = n
  ...
  def tryAdvance(f: java.util.function.LongConsumer): Boolean = 
    if (N <= 0) false
    else {
      if (i >= n) loadMore()
      f.accept(a(i))
  ...
  def trySplit: java.util.Spliterator.OfLong =
    if (N <= 1) null
    else {
      val half = (N >> 1)
