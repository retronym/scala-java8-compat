...
extends AccumulatorLike[Double, DoubleAccumulator]
{ self =>
  ...
  final def iterator = new Iterator[Double] {
    private var consumed = 0L
    private var hix = 0
    private var ix = 0
    private var i = 0
    private var a: Array[Double] = DoubleAccumulator.emptyDoubleArray
  ...
  final def spliterator: java.util.Spliterator.OfDouble = new DoubleAccumulatorSpliterator(this)
...
private[java8] class DoubleAccumulatorSpliterator(private val acc: DoubleAccumulator) extends java.util.Spliterator.OfDouble {
  ...
  private def duplicateSelf(limit: Long = N): DoubleAccumulatorSpliterator = {
    val ans = new DoubleAccumulatorSpliterator(acc)
    ans.h = h
    ans.i = i
    ans.a = a
    ans.n = n
  ...
  def tryAdvance(f: java.util.function.DoubleConsumer): Boolean = 
    if (N <= 0) false
    else {
      if (i >= n) loadMore()
      f.accept(a(i))
  ...
  def trySplit: java.util.Spliterator.OfDouble =
    if (N <= 1) null
    else {
      val half = (N >> 1)
